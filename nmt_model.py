...
class NMT(nn.Module):
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[torch.Tensor, Tuple[torch.Tensor, torch.Tensor]]:
        ...
        X = pack_padded_sequence(input=X, lengths=source_lengths, batch_first=False)    # https://zhuanlan.zhihu.com/p/34418001，可边长rnn
        enc_hiddens, (last_hidden, last_cell) = self.encoder(X)    # X将用于attention，即所有的隐状态
        enc_hiddens, _ = pad_packed_sequence(enc_hiddens, batch_first=False)
        enc_hiddens = enc_hiddens.permute(1, 0, 2)    # 为了后续attention步骤的projection
        last_hidden = torch.cat([last_hidden[0,:,:], last_hidden[1,:,:]], dim=1)    # (2, b, h) -> (b, 2*h)
    ...
    def decode(self, enc_hiddens: torch.Tensor, enc_masks: torch.Tensor,
        ...
        ###      over. Otherwise, you will remove the batch dimension accidentally, if batch_size = 1.       *****************important***************
        ###   
        ...
        for Y_t in ys:    # 在 tgt_len 上切分
            Y_t = torch.squeeze(Y_t, dim=0)    # (1, b, e) -> (b, e)
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        dec_hidden, dec_cell = dec_state    # dec_hidden: (b, h), enc_hiddens_proj: (b, src_len, h)
        e_t = torch.bmm(enc_hiddens_proj, torch.unsqueeze(dec_hidden, dim=2))    # unsqueeze插入一个1维
        e_t = e_t.squeeze(dim=2)    # (b, src_len)
        ### END YOUR CODE

        # Set e_t to -inf where enc_masks has 1
        if enc_masks is not None:
            e_t.data.masked_fill_(enc_masks.byte(), -float('inf'))    # softmax后-inf的部分接近0
        ...
        a_t = torch.bmm(torch.unsqueeze(alpha_t, dim=1), enc_hiddens)    # (b, 1, src_len) * (b, src_len, 2h) = (b, 1, 2h)
        a_t = a_t.squeeze(dim=1)    # (b, 2h)
        U_t = torch.cat([a_t, dec_hidden], dim=1)    # (b, 2h) + (b, h) = (b, 3h)
