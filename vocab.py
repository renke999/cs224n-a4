...
class VocabEntry(object):    # word2idx
    ...
    def to_input_tensor(self, sents: List[List[str]], device: torch.device) -> torch.Tensor:    # 用于构建初始训练数据
        ...
        sents_t = pad_sents(word_ids, self['<pad>'])    # 此处利用第一题所写的oad_sents()函数
        sents_var = torch.tensor(sents_t, dtype=torch.long, device=device)
        return torch.t(sents_var)

    @staticmethod
    def from_corpus(corpus, size, freq_cutoff=2):    # 根据corpus构建出word2idx和idx2word，且词条前面的是出现频率高的
